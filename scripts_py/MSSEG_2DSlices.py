...
from Models import three_layer_depth, four_layer_depth, four_layer_depthDrop, four_layer_2Ddrop
...
############################################### BASELINE UNET ######################################################
####################################################################################################################
def Unet2D(model_arq, inputs, loss, opt):
    with strategy.scope():
        x=inputs
        outputs = model_arq(x)        
...
# Slice indexing
def slc_indx(sample_folds):
    slc_per_smpl = 224
    indexes = np.array([slc_per_smpl*str(i).split(' ') for i in pats]).astype(np.uint8).flatten()
    return indexes
...
# Read
def read_data(dataname, type_):    
    X_patch_name = 'MRIs/X_'+dataname+'_'+type_+'.npy'
    Y_patch_name = 'MRIs/Y_'+dataname+'_'+type_+'.npy'
...
if __name__ == "__main__":  
    ...
    vols, masks = read_data(DATA, 'slices')
    ...
    # Keep slices with at least ?% of tissue, only for training
    tissue_ratio = 0
    ind_x_tr = np.count_nonzero(X_train, axis=(1,2), keepdims=False)>tissue_ratio
    X_train_t = X_train[ind_x_tr]
    y_train_t = y_train[ind_x_tr]

    # Keep slices with at least ?% of lession, only for training
    lession_ratio = 0
    ind_y_tr = np.count_nonzero(y_train, axis=(1,2), keepdims=False)>lession_ratio
    ...
    train_1 = (
        tf.data.Dataset.from_tensor_slices((X_train_t.astype(np.float32), y_train_t.astype(np.uint8)))    
        .batch(BATCH_SIZE)
    ...
    model = Unet2D(eval(m_arquitecture), inputs, DiceLoss, opt) 
